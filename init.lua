...
local function do_nothing(itemstack)
	return itemstack
end
...
minetest.item_place = function(mainhand_stack, player, pointed_thing, ...)
	...
	if not is_switched and (stackname == "" or minetest.registered_tools[stackname] ~= nil) and not inv:get_stack("offhand", 1):is_empty() then
...
controls.register_on_press(function(player, control_name)
	if control_name ~= "place" then
		return
	end
	local mainhand = player:get_wielded_item()
	local stackname = mainhand:get_name() or ""
	-- tool already implements own handler, skip
	if mainhand and mainhand.on_secondary_use then
		return
	end
	-- not actually a tool, skip
	if stackname ~= "" and minetest.registered_tools[stackname] == nil then
		return
	end
	-- looking at ground -> normal on_use will be called, skip
	if get_pointed_thing(player, mainhand) ~= nil then
		return
	end
	return use_offhand(true, mainhand, player, {
		type = "nothing"
	})
end)
